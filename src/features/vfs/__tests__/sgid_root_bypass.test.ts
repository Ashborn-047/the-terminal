...
describe('SGID Root Bypass Prevention', () => {
    ...
    it('should NOT grant full bypass to a user in the root group', async () => {
        // Create a file owned by root:root with mode 600 (rw-------)
        await vfs.touch('/', 'root_secret.txt', 'root');
        await vfs.writeFile('/root_secret.txt', 'classified data', 'root');
        await vfs.chmod('/root_secret.txt', '600', 'root');
    ...
    it('should allow root group access only via group permissions', async () => {
        ...
        await vfs.touch('/', 'group_readable.txt', 'root');
        await vfs.writeFile('/group_readable.txt', 'group data', 'root');
        await vfs.chmod('/group_readable.txt', '640', 'root');
    ...
    it('should deny write to root group member when group write is off', async () => {
        // File with mode 640 — group has read but NOT write
        await vfs.touch('/', 'readonly_group.txt', 'root');
        await vfs.writeFile('/readonly_group.txt', 'original', 'root');
        await vfs.chmod('/readonly_group.txt', '640', 'root');

        // Write attempt by root group member should fail
        const result = await vfs.writeFile('/readonly_group.txt', 'hacked', 'guest', ['root']);
        expect(result).toEqual({ error: 'Permission denied.' });
    ...
    it('should deny access to root group member on mode 600 file', async () => {
        await vfs.touch('/', 'private.txt', 'root');
        await vfs.writeFile('/private.txt', 'secret', 'root');
        await vfs.chmod('/private.txt', '600', 'root');
